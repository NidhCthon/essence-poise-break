...
function cutInPayload(form) {
  ...
  return cleanCutInPayload({
    ...
    name: actor.name,
    img: actor.img,
    color: actor.system?.details?.animacolor,
...
Hooks.once("init", () => {
  ...
  game.settings.register(MODULE_ID, "revealStorytellerCharms", {
    name: "Name the Storyteller's Charms in the cut-in",
    hint: "An antagonist's cut-in shows its portrait but keeps the Charms it "
      + "used from the players, who may not know what it can do. Turn this on "
      + "to name them for everyone.",
    scope: "world",
    config: true,
    type: Boolean,
    default: false
...
Hooks.once("ready", () => {
  ...
  game.socket?.on(CUT_IN_SOCKET, (message) => receiveCutIn(message));
